"""
app.py — E-Commerce Pipeline Data Quality & KPI Dashboard
Reads from marts schema in PostgreSQL.
"""
import os
...
def load_daily_kpis():
    engine = get_engine()
    return pd.read_sql("SELECT * FROM marts.agg_daily_kpis ORDER BY date DESC LIMIT 90", engine)

@st.cache_data(ttl=300)
def load_anomaly_log():
    engine = get_engine()
    return pd.read_sql(
        "SELECT * FROM monitoring.anomaly_log ORDER BY detected_at DESC LIMIT 100", engine
    )
...
def load_pipeline_runs():
    engine = get_engine()
    return pd.read_sql(
        "SELECT * FROM monitoring.pipeline_runs ORDER BY finished_at DESC LIMIT 20", engine
    )
...
def load_customer_segments():
    ...
    return pd.read_sql(
        ...
        "FROM marts.dim_customers GROUP BY customer_segment",
        engine,
    )
...
try:
    ...
    st.info("Make sure PostgreSQL is running and the marts schema is populated via dbt.")
